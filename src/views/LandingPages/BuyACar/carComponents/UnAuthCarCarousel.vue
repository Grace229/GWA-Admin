<template>
    <div class="rounded-lg shadow-md bg-[#ffffff]">
        ...
        <div @click="navigateToSearch(car.financeOptionText, car.id)" class="cursor-pointer flex flex-col gap-3 pt-[21px] px-[19px] bg-[#ffffff]">
            <div class="flex items-center justify-between font-medium text-xs">
                ...
                <h2 class="font-semibold text-sm text-[#30374F]">{{ car.brand }}</h2>
            ...
            <div class="mb-[5px] flex font-normal text-xs text-[#4A5578] gap-[19px]">
                <span class="flex items-center gap-1"
                    ...
                    {{ formatCurrency(car.mileage).slice(1, car.mileage) }}</span
                >
                <span class="flex items-center gap-1"
                    ><svg width="12" height="12" viewBox="0 0 12 12" fill="none" xmlns="http://www.w3.org/2000/svg">
                        <path
...
const navigateToSearch = (finnanceType, carId) => {
    router.push(`/search`);
};
</script>

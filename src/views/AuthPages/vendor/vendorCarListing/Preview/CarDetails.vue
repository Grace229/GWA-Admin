<template>
    <div class="container pt-[51px] pb-[88px]">
        <GoBack class="mb-10" text="Back to results" @click="togglePreview" />
        <div class="flex gap-8">
            <div class="flex flex-1 flex-col">
                <div class="mb-[15px] w-full flex items-center justify-between">
                    <h1 class="font-bold text-[30px] text-[#30374F]">{{ details.year }} {{ details.brand }} {{ details.model }}</h1>

                    <Heart />
                </div>

                <CarGallery class="mb-[29px]" :car="vehicle" />
                ...
                <div class="detailsnfeatures mb-5 grid grid-cols-2 gap-8">
                    <div class="col-span-2 pt-[25px] pb-[38px]">
                        ...
                        <CarSpecs :details="details" :customParams="customParams" />
                    </div>
                </div>
                ...
                <div class="mb-5 detailsnfeatures pt-5 pb-[31px]">
                    ...
                    <div class="flex flex-wrap gap-[15px]">
                        <div v-for="feature in features.items.filter((feature) => feature.checked === true)" :key="feature" class="flex items-center justify-between">
                            <div>
                ...
                <div class="xs-shadow bg-[#ffffff] rounded-[4px] flex border border-[#EFF1F5]">
                    ...
                    <div class="flex-1 p-6">
                        ...
                        <button class="flex items-center rounded-lg gap-[12px] py-[8px] px-[14px] xs-shadow bg-[#4A5578]">
...
const brandId = computed(() => form.brand);
const modelId = computed(() => form.model);
const features = inject('features');
const customParams = computed(() => form.vehicleParameterValues);
const details = ref({
    brand: computed(() => brands.value.filter((brand) => brand.id === brandId.value).map((brand) => brand.text1)[0]),
    model: computed(() => models.value.filter((model) => model.id === modelId.value).map((model) => model.text1)[0]),
    // color: computed(() => form.color),
    // fuelType: computed(() => form.fuelType),
    // engine: computed(() => form.engineType),
    year: computed(() => form.year),
    // interior: computed(() => form.interior),
    transmission: computed(() => Constants.transmissionTypes.find((type) => type.id == form.transmissionType)?.text1),

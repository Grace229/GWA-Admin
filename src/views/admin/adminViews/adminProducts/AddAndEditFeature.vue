        ...
        <Dialog as="div" class="relative z-10" @close="close">
            ...
            <div class="fixed inset-0 z-10 overflow-y-auto">
                <div class="flex min-h-full items-end justify-center p-4 text-center sm:items-center sm:p-0">
                    ...
                    >
                        <DialogPanel class="relative transform overflow-hidden rounded-lg bg-white py-[21px] text-left shadow-xl transition-all sm:my-8 sm:w-full sm:max-w-[400px]">
                            <div>
                                    ...
                                    <div class="col-span-2 grid grid-cols-2 gap-4">
                                        ...
                                        <div class="col-span-2 mt-[14px]">
                                            <button @click="submit" type="button" class="brandButton flex items-center justify-center rounded-lg w-full py-[10px] font-semi-bold text-[14px]">
                                                ...
                                                <SpinnerLoader v-if="featuresActionLoading" class="ml-2 h-[14px] w-[14px]" />
                                            </button>
                                        </div>
                                    </div>
                                </div>
...
const featuresActionLoading = inject('featuresActionLoading');
...
const submit = async () => {
    ...
    if (toggleAddAndEditModalText.value.title === 'Edit Feature') {
        const obj = {
            featureId: toggleAddAndEditModalText.value.feature.id,
            name: form.name,
        };
        emits('proceed', { action: 'Edit Feature', data: obj });
    }

    if (toggleAddAndEditModalText.value.title === 'New Feature') {
        const obj = {
            name: form.name,
        };
        emits('proceed', { action: 'New Feature', data: obj });
    }
};

watch(isAddAndEditOpen, (newValue) => {
    if (newValue === true && toggleAddAndEditModalText.value.title === 'Edit Feature') {
        //populate form inputs edit data

        form.name = toggleAddAndEditModalText.value.feature.name;
    }

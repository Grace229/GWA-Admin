<template>
    <div>
        <div class="flex flex-col gap-[18px] border-b border-[#EAECF0] pb-[19px]">
            <!-- <div class="flex gap-[13px]">
                ...
                <div
                    ...
                    class="px-[10px] py-[2px] rounded-[16px] text-[#027A48] bg-[#ECFDF3] text-sm font-medium max-w-max"
                >
            ...
            <h1 class="text-[#30374F] text-[24px] font-semibold">
                {{ productState.fetchedProductDetails.year }} {{ productState.fetchedProductDetails.brand }} {{ productState.fetchedProductDetails.model }}
            </h1>

            <div class="mb-[5px] flex font-medium text-sm text-[#4A5578] gap-[19px]">
                <span class="flex items-center gap-1"
                    ...
                    {{ productState.fetchedProductDetails.mileage == 0 ? '0.00' : formatCurrency(productState.fetchedProductDetails.mileage).slice(1, productState.fetchedProductDetails.mileage) }}
                    miles</span
                >
                ...
                <span class="flex items-center gap-1"
                    ...
                    {{ productState.fetchedProductDetails.condition }}</span
                >
                <span class="flex items-center gap-1"
                    ><svg width="24" height="24" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
                        <path
                    ...
                    {{ productState.fetchedProductDetails.location }}</span
                >
            </div>
            <h1 class="text-[#1D2939] text-[24px] font-bold mt-[9px]">{{ formatCurrency(productState.fetchedProductDetails.price) }}</h1>
        </div>
        <h1 class="text-[#30374F] text-[18px] font-semibold mt-[21px]">Description</h1>
        <p class="text-[#4A5578] text-sm font-normal mt-[9px]">
            {{ productState.fetchedProductDetails.description }}
        </p>
                ...
                <div class="col-span-2 pt-[25px] pb-[38px]">
                    ...
                    <div class="capitalize">
                        ...
                        <div :class="['border-t border-[#DCDFEA]', 'py-[15px] ']" class="flex items-center justify-between">
                            ...
                            <span class="font-semibold text-sm text-[#30374F]"> {{ productState.fetchedProductDetails.brand }} </span>
                        </div>

                        <div :class="['border-t border-[#DCDFEA]', 'py-[15px] ']" class="flex items-center justify-between">
                            <span class="font-medium text-sm text-[#4A5578]"> Model </span>

                            <span class="font-semibold text-sm text-[#30374F]"> {{ productState.fetchedProductDetails.model }} </span>
                        </div>
                        <div :class="['border-t border-[#DCDFEA]', 'py-[15px] ']" class="flex items-center justify-between">
                            <span class="font-medium text-sm text-[#4A5578]"> Year </span>

                            <span class="font-semibold text-sm text-[#30374F]"> {{ productState.fetchedProductDetails.year }} </span>
                        </div>
                        <div
                            v-for="data in productState?.fetchedProductDetails?.vehicleDetails?.vehicleParameters"
                            :key="data.id"
                        ...
                        <div :class="['border-t border-[#DCDFEA]', 'py-[15px] ']" class="flex items-center justify-between">
                            ...
                            <span class="font-semibold text-sm text-[#30374F]"> {{ productState.fetchedProductDetails.transmissionTypeText }} </span>
                        </div>
                        <div :class="['border-t border-[#DCDFEA]', 'py-[15px] ']" class="flex items-center justify-between">
                            <span class="font-medium text-sm text-[#4A5578]"> Vehicle Id </span>

                            <span class="font-semibold text-sm text-[#30374F]"> {{ productState.fetchedProductDetails.id }} </span>
                        </div>
                        <div :class="['border-t border-[#DCDFEA]', 'py-[15px] ']" class="flex items-center justify-between">
                            <span class="font-medium text-sm text-[#4A5578]"> VIN </span>

                            <span class="font-semibold text-sm text-[#30374F]"> {{ productState.fetchedProductDetails.vin }} </span>
                        </div>
                        <!-- </div> -->
                    </div>
                </div>
        ...
        <div class="mb-5 detailsnfeatures pt-5 pb-[31px]">
            ...
            <div v-if="productState?.fetchedProductDetails?.vehicleDetails?.features.length > 0" class="flex flex-wrap gap-[15px]">
                <div v-for="feature in productState?.fetchedProductDetails?.vehicleDetails?.features" :key="feature.id" class="flex items-center justify-between">
                    <div>

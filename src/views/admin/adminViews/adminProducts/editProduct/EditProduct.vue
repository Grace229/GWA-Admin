<template>
    <div>
        ...
        <!-- {{ v$.$errors }}s -->
        <!-- {{ vehicle.condition }} -->
        <GoBack class="mb-10 mt-2" text="Back to Cars" @click="$router.push('/admin/products')" />

        <SettingsPlate :header="'Edit Car Details'" description="Provide details about your car">
                ...
                <div class="max-w-[728px]">
                    <div class="grid grid-cols-2 gap-x-8 gap-y-5">
                        ...
                        <div class="col-span-1" :class="{ 'input--error': v$.financingType.$errors }">
                            <!-- {{ form }} -->
                            <label for="categories" class="block text-sm font-medium text-[#344054]">Categories </label>
                            <div class="mt-1">
                    ...
                    <div class="mt-[30px] flex justify-end gap-4">
                        ...
                        <!-- <button @click="saveCar" class="cancelButton rounded-[4px] w-[170px] h-10 flex items-center justify-center">Save</button> -->
                        <button @click="editCar" :disabled="loading" :class="loading ? 'opacity-25' : 'opacity-100'" class="brandButton min-w-[170px] h-10 flex items-center justify-center">
                            Save Changes
...
import { editProduct } from '@/services/AdminServices.js';
import { createVehicle, editVehicle } from '@/services/VendorServices.js';
...
// import CustomParamsList from '@/components/reusable/CustomInputs/CustomParamsList.vue';
import AdminProductCustomParamList from '@/views/admin/adminViews/adminProducts/editProduct/AdminProductCustomParamList.vue';
onMounted(() => {
    fetchModels();
});
...
const editCar = async () => {
    console.log('editDetails' + JSON.stringify(editVehicleDetails));

<template>
    <div class="flex flex-col gap-[30px] min-w-[230px]">
        <!-- {{ vehicle }} -->
        <div class="w-full flex flex-col pt-[25px] pb-[31px] px-4 bg-[#F9F9FB] border border-[#DCDFEA] rounded-[5px]">
            <!-- {{ carForEdit.status }} -->
            <!-- <button class="max-w-max bg-[#4E5BA6] text-[#FFFFFF] rounded-[3px] py-[5px] px-[8px] font-medium text-xs mb-[18px]">Draft</button> -->
            <div v-if="Constants.vehicleStatus[carForEdit.status] === 'Draft'" class="bg-[#4A5578] text-[#FFFFFF] rounded-[3px] py-[5px] px-[8px] max-w-max">Draft</div>
            <div v-else-if="Constants.vehicleStatus[carForEdit.status] === 'Published'" class="bg-[#12B76A] text-[#FFFFFF] rounded-[3px] py-[5px] px-[8px] max-w-max">Published</div>
            <div v-else-if="Constants.vehicleStatus[carForEdit.status] === 'Rejected'" class="bg-[#F0203D] text-[#FFFFFF] rounded-[3px] py-[5px] px-[8px] max-w-max">Rejected</div>

            <div v-else-if="Constants.vehicleStatus[carForEdit.status] === 'Pending'" class="bg-[#4E5BA6] text-[#FFFFFF] rounded-[3px] py-[5px] px-[8px] max-w-max">Pending Review</div>
            <div v-else-if="Constants.vehicleStatus[carForEdit.status] === 'Sold'" class="bg-[#12B76A] text-[#FFFFFF] rounded-[3px] py-[5px] px-[8px] max-w-max">Sold</div>

            <div class="mb-[14px] mt-[19px] flex flex-col font-normal text-sm text-[#4A5578] gap-[19px]">
            ...
            <h1 class="font-bold text-[24px] text-[#4A5578]">{{ formatCurrency(vehicle.price) }}</h1>
        </div>
    </div>
</template>
...
const carForEdit = inject('carForEdit');

<template>
    <div :class="!hasActionPermission() ? 'disabled-page' : ''">
                ...
                <div class="-my-2 -mx-6 overflow-x-auto sm:-mx-6 lg:-mx-8">
                    <div class="inline-block min-w-full py-2 align-middle sm:px-6 lg:px-8">
                        <div class="overflow-hidden shadow-[0px_1px_2px_rgba(16,24,40,0.05)] ring-1 ring-black ring-opacity-5 sm:rounded-lg">
                            ...
                            <div class="flex px-4 sm:px-6 justify-between items-center border-b border-[#EAECF0] pt-5 pb-[21px]">
                                <h1 class="font-semibold text-[18px] text-[#1D2939]">Car Features</h1>
                                <button @click="openAddAndEdit('Add', emptyParamObj)" class="gap-3 rounded-lg text-white flex items-center justify-center py-[10px] px-4 bg-[#12B76A]">
                                    ...
                                    <span class="hidden sm:block">Add Feature</span>
                                </button>
                            </div>

                            <!-- /////// -->

                            <div v-if="features?.length === 0 && loadingFeatures">
                                <SpinnerLoader class="mx-auto my-40 h-[50px] w-[50px]" />
                            </div>
                            <div v-else-if="features.length === 0 && !loadingFeatures" class="py-40 w-full bg-white flex flex-col items-center justify-center">
                            ...
                            <table v-else class="min-w-full divide-y divide-[#EAECF0]">
                                ...
                                <tbody class="divide-y divide-gray-200 bg-white">
                                    <tr v-for="(param, index) in features" :key="param.id" :class="index % 2 !== 0 ? undefined : 'bg-gray-50'">
        ...
        <NegativeModal />
        <AddAndEditFeature @proceed="action" />
    </div>
</template>
...
import { getFeatures } from '@/services/AdminServices.js';
import { addFeature, updateFeature } from '@/services/AdminServices.js';
import NegativeModal from '@/views/admin/adminViews/reusable/NegativeModal.vue';
import AddAndEditFeature from '@/views/admin/adminViews/adminProducts/AddAndEditFeature.vue';
onMounted(() => {
    fetchFeatures();
});
...
const openDelete = (feature) => {
    negativeModalText.value.title = `Delete ${feature.name}`;
    negativeModalText.value.feature = feature;
    negativeModalText.value.permanent = true;
    negativeModalText.value.actionName = 'Delete Feature';
    toggleNegativeModal();
};
...
const openAddAndEdit = (type, feature) => {
    if (type === 'Add') {
...
const action = ({ action, data }) => {
    if (action === 'Edit Feature') {
        ...
        updateFeature(
            ...
            (res) => {
                ...
                Util.handleGlobalAlert(true, 'success', 'Feature Successfully Updated');
                toggleAddAndEditModal();
                fetchFeatures();
            },
            (err) => {
    ...
    if (action === 'New Feature') {
        ...
        addFeature(
            ...
            (res) => {
                actionLoading.value = false;
                Util.handleGlobalAlert(true, 'success', 'Feature Successfully Added');
                toggleAddAndEditModal();
                fetchFeatures();
            },
            (err) => {
...
provide('refetch', fetchFeatures);
provide('featuresActionLoading', actionLoading);
</script>

<template>
    <Listbox v-model="selectedOption" class="h-full">
        <div class="relative w-full h-full">
            <label for="address" class="block mb-1 text-sm font-medium text-gray-700">{{ label }}</label>

            <ListboxButton class="relative w-full h-full flex items-center justify-between rounded-[4px] border border-gray-300 focus:border-[#B9C0D4] px-[14px] py-[10px]">
                ...
                >
                <p v-else class="mr-2 font-normal text-sm text-[#5D6B98]">Select {{ label }}</p>
                <div>

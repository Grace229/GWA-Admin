<template>
    <Listbox v-model="selectedOption" class="h-full bg-[#ffffff]">
        <div class="relative w-full h-full">
            ...
            <ListboxButton class="relative w-full h-full flex items-center justify-between rounded-[4px] border border-[#B9C0D4] px-[14px] py-[10px] ring-0 outline-none">
                <span class="block text-sm" v-if="selectedOption">
                    <div class="text-sm text-[#3A3745] flex items-center gap-x-1 capitalize">
            ...
            <transition leave-active-class="transition duration-100 ease-in" leave-from-class="opacity-100" leave-to-class="opacity-0">
                <ListboxOptions
                    class="absolute max-h-40 w-full mt-[.5rem] z-[999] overflow-auto rounded-lg bg-white text-sm shadow-[0px_12px_16px_-4px_rgba(16,24,40,0.08),_0px_4px_6px_-2px_rgba(16,24,40,0.03)] ring-1 ring-black ring-opacity-5 border-b-2 outline-0 sm:text-sm"
                >

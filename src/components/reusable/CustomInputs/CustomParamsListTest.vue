    ...
    <Combobox as="div" v-model="selectedOption">
        ...
        <div class="relative">
            ...
            <ComboboxInput
                ...
                :display-value="(option) => (option ? option : '')"
                :placeholder="label"
            />
...
const props = defineProps(['label', 'options', 'id', 'modelValue', 'selectedParams']);
const emits = defineEmits(['onGetData', 'update:modelValue', 'addItem']);
const selectedOption = ref(null);
const setOption = () => {
    const foundItem = props.selectedParams.find((item) => item.parameterId === props.id);

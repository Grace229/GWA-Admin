<template>
    <TransitionRoot as="template" :show="open">
        <Dialog as="div" class="relative z-10" @close="toggleAddModelModal">
                ...
                <div class="flex min-h-full items-end justify-center p-4 text-center sm:items-center sm:p-0">
                    ...
                    >
                        <DialogPanel class="relative transform overflow-hidden rounded-lg bg-white pt-5 pb-4 text-left shadow-xl transition-all sm:my-8 sm:w-full sm:max-w-[400px] sm:py-6">
                            <div class="border-b pb-3 mb-4 px-4">Add Model</div>

                            <div class="px-4 grid grid-cols-1 gap-4">
                                <div class="col-span-">
                                    <label for="name" class="block text-sm font-medium text-[#344054]">Model Name</label>
                                    <div class="mt-1">
                                        <input
                                            type="text"
                                            name="name"
...
import { createModel } from '@/services/MarketPlace.js';
const open = inject('isAddModelModalOpen');
const parentForm = inject('form');
const getModels = inject('getModels');
const toggleAddModelModal = inject('toggleAddModelModal');
const loading = ref(false);
const logoUrlName = ref('');
const form = reactive({
    name: '',
    brandId: computed(() => parentForm.brand),
});
